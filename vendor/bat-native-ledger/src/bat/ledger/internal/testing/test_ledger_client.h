...
class TestLedgerClient : public LedgerClient {
 public:
  ...
  // All non-virtual methods should be named with a ForTesting suffix for
  // clarity and in order to avoid name collisions with LedgerClient.
  ...
  const base::Value& state_store_for_testing() const {
    return state_store_;
  }
